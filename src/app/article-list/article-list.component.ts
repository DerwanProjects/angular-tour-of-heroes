...
export class ArticleListComponent implements OnInit {
  ...
  makeUnixTimestamp(dateString: string): number {
    ...
    let date = new Date(dateArray[2], dateArray[1] - 1, dateArray[0]); // zwróć sekwencję year,month,day
    let unixDate = date.getTime();
    return unixDate;
  }
  // Tu trzeba się upewnić aby ta operacja sortowania wykonywała się tylko 1 raz
  postzSorted = this.postz.sort((a,b) => this.makeUnixTimestamp(a.date) - this.makeUnixTimestamp(b.date) );
